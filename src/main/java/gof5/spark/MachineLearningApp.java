...
public class MachineLearningApp {
	...
	private static final org.slf4j.Logger log = LoggerFactory.getLogger(MachineLearningApp.class);
	...
	private static final String INPUT_TABLE_ID = "charged-thought-310615:1111.test";
...
//        String outputModel = args[1];
		...
		Configuration bigQueryConf = getBigQueryConf(sc.hadoopConfiguration());
		...
		JavaPairRDD<LongWritable, JsonObject> tableData = sc.newAPIHadoopRDD(
				bigQueryConf,
				GsonBigQueryInputFormat.class,
				LongWritable.class,
				JsonObject.class);

		log.info(tableData.values().toDebugString());
...
//		log.info("Model Prediction on New Data = " + prediction);
	...
	public static Configuration getBigQueryConf(Configuration configuration) throws IOException {
		configuration.set(BigQueryConfiguration.PROJECT_ID_KEY, configuration.get(FS_GS_PROJECT_ID));
		//configuration.set(BigQueryConfiguration.GCS_BUCKET_KEY, configuration.get(FS_GS_SYSTEM_BUCKET));
		BigQueryConfiguration.configureBigQueryInput(configuration, INPUT_TABLE_ID);
